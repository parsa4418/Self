...
def add_chat_word_filter(
    ...
    if not phrase:
        raise ValueError("کلمه فیلتر نمی‌تواند خالی باشد.")
    with connect(db_path) as connection:
        try:
            cursor = connection.execute(
                """INSERT INTO chat_word_filters(chat_id, phrase, action)
                   VALUES (?, ?, ?)
                   ON CONFLICT(chat_id, phrase) DO UPDATE SET
                       action = excluded.action,
                       is_active = 1""",
                (int(chat_id), phrase, str(action)),
            )
        except sqlite3.IntegrityError as exc:
            raise ValueError("این کلمه قبلاً در فیلتر همین گروه ثبت شده است.") from exc
        row = connection.execute(
            "SELECT id FROM chat_word_filters WHERE chat_id = ? AND phrase = ?",
            (int(chat_id), phrase),
        ).fetchone()
    return int(row["id"]) if row else int(cursor.lastrowid or 0)


def list_chat_word_filters(
    data_dir: str | Path,
    phone: str,
    chat_id: int,
    limit: int = 200,
...
def delete_chat_word_filter(
    ...
    with connect(db_path) as connection:
        cursor = connection.execute(
            "DELETE FROM chat_word_filters WHERE chat_id = ? AND id = ?",
            (int(chat_id), int(filter_id)),
        )
