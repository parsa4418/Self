...
class TelegramAuthBot(AdminPanelMixin):
    def __init__(self, token, api_id, api_hash):
        ...
        self.self_start_timeout = max(90, int(os.getenv("SELF_START_TIMEOUT", "180")))
        self.application = (
            Application.builder()
            .token(token)
    ...
    def permanent_selfbot_failure(self, detail: str) -> bool:
        ...
        *,
        ...
        async with self.self_operation_lock(user_id):
            ...
            try:
                ...
                # The one-time activation message is sent only during the
                # initial activation. Watchdog restarts must not spam Saved
                # Messages with a new startup card.
                if reason == "activation":
                    command.extend(("--send-startup-message", "--reset-startup-message"))
    ...
    async def reconcile_selfbots(self) -> None:
        for record in self.registered_selfbot_rows():
            ...
            if existing and not existing.done():
                continue
            # Only a previously healthy/running selfbot is eligible for
            # automatic recovery. Startup/activation failures must not enter
            # an endless watchdog loop on their own.
    ...
    async def send_code_entry_prompt(
        ...
        try:
            ...
            # Cosmetic only: never block the actual login-code prompt.
            logging.info("Verification progress edit skipped: %s", type(exc).__name__)
        ...
        try:
            ...
            logging.exception("Could not send verification code prompt")
            # Last-resort fallback through the bot API.
