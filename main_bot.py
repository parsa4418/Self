...
class TelegramAuthBot(AdminPanelMixin):
    ...
    def apply_feature_policies_for_user(self, user_id: int, phone: str) -> None:
        """Initialize an activated self-bot with opt-in feature defaults."""
        db_path = ensure_self_settings(DATA_DIR, str(phone))
        try:
            with db_connect(db_path, timeout=10) as conn:
                ...
                off_keys = (
                    ...
                    "anti_delete_groups", "anti_delete_channels",
                ...
                for key in off_keys:
                    conn.execute(
                        "INSERT INTO settings(key, value) VALUES (?, 'off') "
                        "ON CONFLICT(key) DO UPDATE SET value = 'off'",
                        (key,),
                    )
        except Exception:
            logging.exception("Could not initialize safe feature policies for %s", user_id)
    ...
    async def verify_code_text(
        ...
        # پیام کد ورود عمداً حذف نمی‌شود.
    ...
    async def complete_selfbot_activation(self, user_id, session_data, reply_message):
        ...
        try:
            # Reset every opt-in feature before the self-bot process starts.
            # This prevents stale settings from older databases from executing
            # during the first seconds of activation.
    ...
    def record_selfbot_failure(self, user_id, reason, permanent=None):
        """Record a self-bot failure without breaking watchdog restarts."""
        try:
            detail = str(reason or "خطای نامشخص")
            if permanent is None:
                permanent = self.permanent_selfbot_failure(detail)
            values = {
                "self_pid": None,
                "self_last_error": detail[:2000],
                "self_status": "permanent_failure" if permanent else "failed",
                "self_last_stopped_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "self_next_restart_at": None,
            }
            if permanent:
                values["self_enabled"] = 0
            self.update_selfbot_runtime(int(user_id), **values)
    ...
    def run(self):
        ...
        port = int(os.getenv("INTERNAL_WEBHOOK_PORT", "10001"))
