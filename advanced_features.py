...
class AdvancedFeatureEngine:
    ...
    async def handle_command(self, event) -> bool:
        ...
        match = re.fullmatch(
            r"(?:خوش آمد|خوش آمد گویی)\s+(روشن|خاموش)",
            lower,
        )
        if match:
            state = self.on_off(match.group(1))
        ...
        match = re.fullmatch(
            r"(?:متن خوش آمد|تنظیم متن خوش آمد گویی)\s+([\s\S]+)",
            raw,
            re.IGNORECASE,
        )
        if match:
        ...
        match = re.fullmatch(
            r"(?:متن خداحافظی|تنظیم متن خداحافظی)\s+([\s\S]+)",
            raw,
            re.IGNORECASE,
        )
        if match:
        ...
        match = re.fullmatch(
            r"(?:فونت نام|تنظیم فونت ساعت اسم)\s+([1-9]|10)",
            lower,
        )
        if match:
        ...
        match = re.fullmatch(
            r"(?:فونت بیو|تنظیم فونت ساعت بیو)\s+([1-9]|10)",
            lower,
        )
        if match:
    ...
    async def _persistent_action_loop(self, chat_id: int, action: str) -> None:
        try:
            while True:
            ...
            self.record_error(f"persistent action {action}: {type(exc).__name__}: {exc}")

    async def start_persistent_action(self, event, action: str) -> None:
        chat_id = int(event.chat_id)
        await self.stop_persistent_action(event, silent=True)
        task = asyncio.create_task(self._persistent_action_loop(chat_id, action))
        self._active_action_tasks[chat_id] = task
    ...
    async def stop_persistent_action(self, event, silent: bool = False) -> None:
        chat_id = int(event.chat_id)
        task = self._active_action_tasks.pop(chat_id, None)
