    ...
    async def handle_command(self, event) -> bool:
        ...
        if match:
            ...
            try:
                ...
                await self.safe_edit(
                    event,
                    f"⚠️ تلگرام ارسال را برای {max(1, int(getattr(exc, 'seconds', 1)))} ثانیه محدود کرد.\n"
                    f"تعداد ارسال‌شده: {sent}"
                )
                return True
            except Exception as exc:
                await self.safe_edit(
                    event,
                    f"❌ ارسال متوقف شد.\nتعداد ارسال‌شده: {sent}\n"
