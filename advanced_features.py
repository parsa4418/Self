...
ACTION_ALIASES = {
    "تایپ": "typing", "typing": "typing",
    "ویس": "record-audio", "voice": "record-audio",
    "صدا": "audio", "audio": "audio", "song": "audio",
    "ویدیو": "record-video", "ویدئو": "record-video", "video": "record-video",
    "عکس": "photo", "photo": "photo",
    "فایل": "document", "file": "document",
    "استیکر": "sticker", "sticker": "sticker",
    "موقعیت": "location", "location": "location",
    "بازی": "game", "game": "game",
}
ACTION_SETTING_KEYS = {
    "typing": "action_typing", "game": "action_game",
    "record-audio": "action_voice", "audio": "action_audio",
    "record-video": "action_video", "photo": "action_photo",
    "document": "action_file", "location": "action_location",
    "sticker": "action_sticker",
}
...
class AdvancedFeatureEngine:
    ...
    async def register_handlers(self) -> None:
        if not any(getattr(task, "get_name", lambda: "")() == "self-action-loop" for task in self.background_tasks):
            self.background_tasks.append(asyncio.create_task(self.action_status_loop(), name="self-action-loop"))
    ...
    async def action_status_loop(self) -> None:
        """Keep the selected Telegram chat action visible in group chats while enabled."""
        action_to_setting = {
            "typing": "action_typing",
            "game": "action_game",
            "record-audio": "action_voice",
            "audio": "action_audio",
            "record-video": "action_video",
            "photo": "action_photo",
            "document": "action_file",
            "geo": "action_location",
            "sticker": "action_sticker",
        }
        while not getattr(self.account, "shutdown_requested", False):
            try:
                if not getattr(self.account, "is_running", False):
                    await asyncio.sleep(1)
                    continue
                settings = self.settings()
                selected = next((action for action, key in action_to_setting.items() if settings.get(key) == "on"), None)
                if not selected:
                ...
                async for dialog in self.client.iter_dialogs(limit=100):
                    if not getattr(dialog, "is_group", False):
                        continue
                    try:
                        async with self.client.action(dialog.entity, selected):
                ...
                self.record_error(f"action-loop {type(exc).__name__}: {exc}")
    ...
    async def enforce_private_lock(self, event) -> bool:
        ...
        # Incoming private messages are never deleted automatically.
        # The setting is retained only for backward compatibility with old DBs.
        return True
    ...
    async def handle_command(self, event) -> bool:
        ...
        match = re.fullmatch(
            r"(?:ضد ویرایش|محافظ ویرایش|anti edit)\s+(پیوی|گروه|private|group)\s+"
            r"(روشن|خاموش|on|off)",
            lower,
        )
        if match:
        ...
        match = re.fullmatch(
            r"(?:اکشن|action)\s+(تایپ|بازی|ویس|صدا|ویدیو|ویدئو|عکس|فایل|موقعیت|استیکر|typing|game|voice|audio|video|photo|file|location|sticker)\s+(روشن|خاموش|on|off)",
            lower,
        )
        if match:
            action_name = ACTION_ALIASES.get(match.group(1))
            key = ACTION_SETTING_KEYS.get(action_name or "")
            if key:
                state = self.on_off(match.group(2)) or "off"
                if state == "on":
                    values = {name: "off" for name in ACTION_SETTING_KEYS.values()}
                    values[key] = "on"
                    self.save_settings(values)
                else:
                    self.save_settings({key: "off"})
                await self.safe_edit(event, f"✅ اکشن {match.group(1)} {'فعال' if state == 'on' else 'خاموش'} شد.")
                return True

        if lower in {"اکشن خاموش", "action off"}:
            self.save_settings({name: "off" for name in ACTION_SETTING_KEYS.values()})
    ...
    async def show_action(
        ...
        settings = self.settings()
        setting_key = ACTION_SETTING_KEYS.get(action)
        if setting_key and settings.get(setting_key, "off") != "on":
            await self.safe_edit(event, "❌ این اکشن خاموش است. ابتدا آن را روشن کنید.")
            return
    ...
    async def create_qr(self, event, text: str) -> None:
        ...
        # پیام دستور QR عمداً حذف نمی‌شود.
