...
class TelegramAccount:
    def __init__(self, phone, session_string, account_manager, status_file=None):
        ...
        self.send_startup_message_once = False
    ...
    async def safe_pm_cleanup(self):
        """No automatic dialog/message deletion on startup."""
        return
    ...
    def startup_message_was_sent(self) -> bool:
        try:
            return str(self.get_data().get("startup_message_sent", "0")).strip() == "1"
        except Exception:
            return False

    def mark_startup_message_sent(self) -> None:
        try:
            settings = self.get_data()
            settings["startup_message_sent"] = "1"
            self.put_data(settings)
        except Exception as exc:
            print(f"⚠️ ثبت وضعیت پیام فعال‌سازی برای {self.phone} ناموفق بود: {exc}")
        ...
        async def handle_outgoing_messages(event):
            try:
                ...
                handlers = {
                    ...
                    '.وضعیت سلف': self.status_handler,
                    'وضعیت': self.settings_status_handler,
                    '.وضعیت': self.settings_status_handler,
...
✅ **اکانت با موفقیت غیرفعال شد**
    ...
    async def settings_status_handler(self, event):
        """نمایش وضعیت قابلیت‌های تنظیم‌شده سلف."""
        try:
            js = self.get_data()
            labels = [
                ("همیشه آنلاین", "online_status"),
                ("تایپینگ", "typing_action"),
                ("منشی عمومی", "secretary"),
                ("منشی", "offline_secretary"),
                ("ذخیره عکس زمان‌دار", "save_timed_photos"),
                ("امضای پیام", "outgoing_signature_enabled"),
                ("عضویت اجباری", "force_join_private"),
                ("سین خودکار پیوی", "auto_read_private"),
                ("سین خودکار گروه", "auto_read_groups"),
                ("سین خودکار کانال", "auto_read_channels"),
                ("ری‌اکت خودکار", "auto_reaction"),
                ("فیلتر کلمه", "word_filter_enabled"),
            ]
            lines = ["⚙️ **وضعیت تنظیمات سلف**", "━━━━━━━━━━━━━━", ""]
            for title, key in labels:
                value = js.get(key, "off")
                lines.append(f"• {title}: {'✅ فعال' if value == 'on' else '❌ غیرفعال'}")
            await event.edit("\n".join(lines))
        except Exception as exc:
            print(f"خطا در وضعیت تنظیمات برای {self.phone}: {exc}")
    ...
    async def status_handler(self, event):
        ...
        try:
            ...
            txt = (
                ...
                f"• منشی: {state('offline_secretary')}\n"
    ...
    async def register_settings_handlers(self):
        ...
        @self.client.on(events.NewMessage(pattern=r'^\.?(?:حالت آنلاین|حالت تایپینگ|منشی عمومی|ساعت اسم|ساعت بیو|ذخیره عکس زمان‌دار) (?:روشن|خاموش)$'))
        async def unified_persian_settings_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                raw = (event.raw_text or '').strip().lstrip('.')
                parts = raw.rsplit(' ', 1)
                command, state_word = parts[0], parts[1]
                value = 'on' if state_word == 'روشن' else 'off'
                mapping = {
                    'حالت آنلاین': 'online_status', 'حالت تایپینگ': 'typing_action',
                    'منشی عمومی': 'secretary', 'ساعت اسم': 'timename',
                    'ساعت بیو': 'timebio', 'ذخیره عکس زمان‌دار': 'save_timed_photos',
                }
                key = mapping[command]
                js = self.get_data(); js[key] = value; self.put_data(js)
                if key == 'online_status':
                    if value == 'on': await self.set_online_status()
                    else:
                        self.observed_presence_online = None
                        await self.apply_presence_name_emoji(force=True)
                elif key in {'timename', 'timebio'}:
                    await self.force_time_update()
                await event.edit(f"{command} {'فعال' if value == 'on' else 'غیرفعال'} شد ✓")
            except Exception as exc:
                print(f"خطا در تنظیم فارسی برای {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?حالت (آنلاین|تایپینگ) (روشن|خاموش)$'))
        async def persian_presence_typing_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                name = event.pattern_match.group(1)
                state = event.pattern_match.group(2)
                key = 'online_status' if name == 'آنلاین' else 'typing_action'
                value = 'on' if state == 'روشن' else 'off'
                js = self.get_data()
                js[key] = value
                self.put_data(js)
                if key == 'online_status':
                    if value == 'on':
                        ...
                        self.observed_presence_online = None
                        await self.apply_presence_name_emoji(force=True)
                await event.edit(f"{name} {'فعال' if value == 'on' else 'غیرفعال'} شد ✓")
            except Exception as exc:
                print(f"خطا در تنظیم {name}: {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?منشی عمومی (روشن|خاموش)$'))
        async def persian_general_secretary_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = 'on' if event.pattern_match.group(1) == 'روشن' else 'off'
                js = self.get_data()
                js['secretary'] = value
                self.put_data(js)
                await event.edit(f"منشی عمومی {'فعال' if value == 'on' else 'غیرفعال'} شد ✓")
            except Exception as exc:
                print(f"خطا در تنظیم منشی عمومی برای {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?ذخیره عکس زمان[‌ ]?دار (روشن|خاموش)$'))
        async def persian_timed_photo_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = 'on' if event.pattern_match.group(1) == 'روشن' else 'off'
                js = self.get_data()
                js['save_timed_photos'] = value
                self.put_data(js)
                await event.edit(f"ذخیره عکس زمان‌دار {'فعال' if value == 'on' else 'غیرفعال'} شد ✓")
            except Exception as exc:
                print(f"خطا در تنظیم ذخیره عکس زمان‌دار برای {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?تنظیم متن منشی (.+)$'))
        async def persian_secretary_text_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                text = event.pattern_match.group(1).strip()[:3500]
                js = self.get_data(); js['secretary_fallback_text'] = text; self.put_data(js)
                await event.edit('متن منشی ذخیره شد ✓')
            except Exception as exc:
                print(f"خطا در تنظیم متن منشی برای {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?منشی (روشن|خاموش)$'))
        async def persian_offline_secretary_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = 'on' if event.pattern_match.group(1) == 'روشن' else 'off'
                js = self.get_data()
                js['offline_secretary'] = value
                self.put_data(js)
                await event.edit(f"منشی {'فعال' if value == 'on' else 'غیرفعال'} شد ✓")
            except Exception as exc:
                print(f"خطا در تنظیم منشی برای {self.phone}: {exc}")

        @self.client.on(events.NewMessage(pattern=r'^\.?تنظیم زمان تایپینگ (\d+)$'))
        ...
        @self.client.on(events.NewMessage(pattern=r'^\.?تنظیم فونت ساعت (اسم|بیو) (\d+)$'))
        async def named_clock_font_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                target = event.pattern_match.group(1)
                raw_num = self.normalize_digits(event.pattern_match.group(2))
                font_num = int(raw_num)
                if not 1 <= font_num <= 10:
                    await event.edit("❌ شماره فونت باید بین ۱ تا ۱۰ باشد.")
                    return
                settings = self.get_data()
                key = "timename_font" if target == "اسم" else "timebio_font"
                settings[key] = str(font_num)
                self.put_data(settings)
                await self.force_time_update()
                label = "اسم" if target == "اسم" else "بیو"
                await event.edit(f"✅ فونت ساعت {label} روی شماره {font_num} تنظیم شد.")
            except Exception as exc:
                print(f"خطا در تنظیم فونت ساعت {self.phone}: {exc}")
        ...
        async def persian_welcome_goodbye_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                ...
                enabled_key = (
                    "welcome_enabled"
                    if joined
                    else "goodbye_enabled"
                )
                if settings.get(enabled_key) != "on":
                    return
                target_user = None
                try:
                ...
                text = str(settings.get(template_key, "") or "").strip()
                if not text:
                    text = (
                        f"👋 {name} به گروه خوش اومدی!"
                        if joined
                        else f"👋 {name} از گروه خارج شد."
                    )
                ...
                print(
                    f"خطا در خوش‌آمد/خداحافظی برای {self.phone}: "
        ...
        @self.client.on(
            events.NewMessage(
                outgoing=True,
                pattern=r"^\.?خوش\s+آمد\s+گویی\s+(روشن|خاموش)$",
            )
        )
        async def welcome_toggle_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = "on" if event.pattern_match.group(1) == "روشن" else "off"
                settings = self.get_data()
                settings["welcome_enabled"] = value
                self.put_data(settings)
                await event.edit(
                    f"خوش آمد گویی {'فعال' if value == 'on' else 'غیرفعال'} شد ✓"
                )
            except Exception as exc:
                print(f"خطا در تنظیم خوش آمد گویی: {exc}")

        @self.client.on(
            events.NewMessage(
                outgoing=True,
                pattern=r"^\.?خوش\s+آمد\s+گویی\s+(.+)$",
            )
        )
        async def welcome_text_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = event.pattern_match.group(1).strip()[:3500]
                settings = self.get_data()
                settings["welcome_text"] = value
                self.put_data(settings)
                await event.edit("متن خوش آمد گویی ذخیره شد ✓")
            except Exception as exc:
                print(f"خطا در متن خوش آمد گویی: {exc}")

        @self.client.on(
            events.NewMessage(
                outgoing=True,
                pattern=r"^\.?خداحافظی\s+(روشن|خاموش)$",
            )
        )
        async def goodbye_toggle_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = "on" if event.pattern_match.group(1) == "روشن" else "off"
                settings = self.get_data()
                settings["goodbye_enabled"] = value
                self.put_data(settings)
                await event.edit(
                    f"خداحافظی {'فعال' if value == 'on' else 'غیرفعال'} شد ✓"
                )
            except Exception as exc:
                print(f"خطا در تنظیم خداحافظی: {exc}")

        @self.client.on(
            events.NewMessage(
                outgoing=True,
                pattern=r"^\.?تنظیم\s+متن\s+خداحافظی\s+(.+)$",
            )
        )
        async def goodbye_text_handler(event):
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                value = event.pattern_match.group(1).strip()[:3500]
                settings = self.get_data()
                settings["goodbye_text"] = value
                self.put_data(settings)
                await event.edit("متن خداحافظی ذخیره شد ✓")
            except Exception as exc:
                print(f"خطا در متن خداحافظی: {exc}")
    ...
    async def check_expiration(self):
        ...
        # Grace period so activation can finish before validity is checked.
    ...
    def is_self_valid(self):
        ...
        try:
            ...
            if not result:
                ...
                return False
            expiration_text, is_active = result
            # Once this process is running, the real Telegram Client is the
            # runtime authority.  self_enabled is a control flag owned by the
            # manager/watchdog and must not make an already-running client
            # fail its own connection validation.
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="ارسال یک‌باره پیام فعال‌سازی اولیه؛ در restart فعال نمی‌شود.",
    )
    parser.add_argument(
        "--reset-startup-message",
        action="store_true",
        help="پاک‌کردن علامت پیام فعال‌سازی برای یک فعال‌سازی جدید.",
    )
...
async def main():
    ...
    account.send_startup_message_once = bool(args.send_startup_message)
    if args.reset_startup_message:
        try:
            settings = account.get_data()
            settings["startup_message_sent"] = "0"
            account.put_data(settings)
        except Exception as exc:
            print(f"⚠️ ریست وضعیت پیام فعال‌سازی برای {args.phone} ناموفق بود: {exc}")
    await account.run()
