        ...
        try:
            ...
            if self.client is not None:
                try:
                    ...
                    pass
            # A normal Telethon disconnect is a runtime event, not a failed
            # login.  The manager/watchdog decides whether and when to restart
            # the saved session.
            if not self.shutdown_requested:
                write_runtime_status(
                    self.status_file,
                    "disconnected",
                    self.last_startup_error or "اتصال سلف قطع شد",
                )
