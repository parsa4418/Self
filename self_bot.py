...
class TelegramAccount:
    ...
    async def safe_initialize_client(self):
        """Connect the single persistent client used by this selfbot process."""
        self.last_startup_error = ""
        # A retry must never leave the previous Telethon client alive.
        if self.client is not None:
            ...
            self.client = None
        try:
            print(f"🔄 در حال راه‌اندازی اکانت {self.phone}...", flush=True)
            write_runtime_status(self.status_file, "starting", "ساخت کلاینت تلگرام")
            ...
            # اتصال با timeout
            write_runtime_status(self.status_file, "starting", "اتصال به تلگرام")
            await asyncio.wait_for(self.client.connect(), timeout=30)
            
            write_runtime_status(self.status_file, "starting", "بررسی اعتبار سشن")
            ...
            try:
                write_runtime_status(self.status_file, "starting", "بررسی حساب تلگرام")
    ...
    async def robust_initialize(self):
        """Complete startup once; retries cleanly without stacking clients/tasks."""
        for attempt in range(self.max_retries):
            try:
                print(
                    f"🔄 تلاش {attempt + 1}/{self.max_retries} برای راه‌اندازی {self.phone}",
                    flush=True,
                )
                if not await self.safe_initialize_client():
                    raise RuntimeError(
                        self.last_startup_error or "اتصال کلاینت ناموفق بود"
                    )

                write_runtime_status(self.status_file, "starting", "بارگذاری تنظیمات")
                self.init_db()
                ...
                try:
                    min_interval = max(
                        ...
                        min(
                            5000,
                            int(settings.get("send_queue_min_interval_ms", "900")),
                        ),
                    )
                except (TypeError, ValueError):
                    min_interval = 900
                ...
                write_runtime_status(self.status_file, "starting", "ثبت قابلیت‌های سلف")
                ...
                # Only the explicit first activation can send this message.
                ...
                # Telethon already has auto_reconnect enabled. The health task
                # is observational; it must not force disconnect/reconnect cycles.
                self.start_background_task(
                ...
                write_runtime_status(self.status_file, "ready", "سلف آماده و در حال اجرا است")
                print(f"✅ اکانت {self.phone} با موفقیت راه‌اندازی شد", flush=True)
                return True
                ...
                write_runtime_status(
                    self.status_file,
                    "starting",
                    f"تلاش {attempt + 1}/{self.max_retries}: {self.last_startup_error}",
                )
                print(
                    f"❌ خطا در راه‌اندازی (تلاش {attempt + 1}): "
                    f"{self.last_startup_error}",
                    flush=True,
                )
                self.is_running = False
                self.shutdown_requested = False
                ...
                if attempt + 1 < self.max_retries:
                    await asyncio.sleep(min(15 * (attempt + 1), 45))

        write_runtime_status(
            self.status_file,
            "failed",
            self.last_startup_error or "راه‌اندازی کلاینت تلگرام ناموفق بود",
        )
        return False

    async def health_monitor(self):
        """Observe health without fighting Telethon's built-in auto-reconnect."""
        while self.is_running and not self.shutdown_requested:
            try:
                ...
                if self.client.is_connected():
                    continue
                # A transient network loss is handled by Telethon's
                # auto_reconnect. Do not call disconnect()/connect() here.
                print(
                    f"🔌 اتصال {self.phone} موقتاً قطع است؛ "
                    "Telethon در حال بازیابی خودکار است.",
                    flush=True,
                )
                ...
                print(
                    f"⚠️ خطا در پایش سلامت {self.phone}: "
                    f"{type(exc).__name__}: {exc}",
                    flush=True,
                )
    ...
    async def perform_health_check(self):
        try:
            ...
            print(
                f"⚠️ بررسی سلامت {self.phone} ناموفق بود: "
                f"{type(exc).__name__}: {exc}",
                flush=True,
            )
            return False
    ...
    async def recover_connection(self):
        """Compatibility shim: let Telethon auto-reconnect the existing client."""
        if self.client is None or self.shutdown_requested or not self.is_running:
            return False
    ...
    async def run(self):
        ...
        try:
            ...
            if success:
                ...
                print(f"🚀 اکانت {self.phone} در حال اجرا است...", flush=True)
                # Keep the same authenticated Telethon client alive if the
                # transport drops unexpectedly. Telethon normally reconnects
                # by itself, but run_until_disconnected() can return after a
                # disconnect; that must not terminate the whole selfbot process
                # and trigger the controller watchdog. Only an explicit shutdown
                # is allowed to leave this loop.
                reconnect_delay = 2
                while self.is_running and not self.shutdown_requested:
                    try:
                    ...
                    print(
                        ...
                        f"تلاش برای اتصال مجدد در {reconnect_delay} ثانیه...",
                        flush=True,
                    )
                    ...
                    try:
                        ...
                        if await self.client.is_user_authorized():
                            ...
                            write_runtime_status(
                                self.status_file,
                                "ready",
                                "اتصال سلف بازیابی شد و در حال اجرا است",
                            )
                            print(
                                f"✅ اتصال {self.phone} دوباره برقرار شد",
                                flush=True,
                            )
                        else:
                            detail = "سشن تلگرام پس از قطع اتصال دیگر معتبر نیست"
                            write_runtime_status(
                                self.status_file, "failed", detail
                            )
                            self.last_startup_error = detail
                            break
                    except asyncio.CancelledError:
                        raise
                    except Exception as exc:
                        print(
                            f"⚠️ اتصال مجدد {self.phone} ناموفق بود: "
                            f"{type(exc).__name__}: {exc}",
                            flush=True,
                        )
