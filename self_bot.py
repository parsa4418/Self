...
class TelegramAccount:
    ...
    async def safe_pm_cleanup(self):
        """حذف خودکار پیام/گفتگو عمداً غیرفعال است."""
        return
    ...
    async def send_login_notification(self):
        """ثبت اطلاعیه فعال‌سازی فقط در Saved Messages."""
        try:
            me = await self.client.get_me()
...
🔥 **𝙶𝚛𝚘𝚙:** {self.brand_username('brand_group')}
            """
            await self.queued_send_message("me", login_message, priority=20)
            print(f"✅ اطلاعیه لاگین فقط در Saved Messages برای {self.phone} ثبت شد")
        except Exception as e:
            print(f"خطا در ارسال اطلاعیه لاگین برای {self.phone}: {e}")

    def apply_one_time_feature_defaults(self) -> None:
        """یک‌بار همه قابلیت‌های خودکار اختیاری را خاموش می‌کند."""
        try:
            settings = self.get_data()
            if settings.get("feature_defaults_reset_v4") == "1":
                return
            off_keys = {
                "comment", "online_status", "save_timed_photos",
                "friend_affection_reply", "enemy_hostile_reply",
                "anti_delete_enabled", "anti_delete_private",
                "anti_delete_groups", "anti_delete_channels", "lock_audio",
                "auto_read_private", "auto_read_groups", "auto_read_channels",
                "auto_reaction", "relationship_reaction", "force_join_private",
                "typing_action", "scheduled_message_enabled", "auto_forward",
                "outgoing_signature_enabled", "word_filter_enabled",
                "first_comment_enabled", "welcome_enabled", "goodbye_enabled",
                "private_lock_enabled", "private_lock_delete_unknown",
                "private_lock_warn_before_block", "anti_edit_private",
                "anti_edit_groups", "anti_edit_notify_saved", "profile_monitor_enabled",
                "analog_clock_enabled", "presence_emoji_enabled",
                "form_builder_enabled", "profile_backup_enabled",
                "action_typing", "action_game", "action_voice", "action_audio",
                "action_video", "action_photo", "action_file",
                "action_location", "action_sticker",
            }
            self.put_data({**{key: "off" for key in off_keys},
                           "feature_defaults_reset_v4": "1"})
        except Exception as exc:
            print(f"⚠️ مهاجرت تنظیمات امن {self.phone} ناموفق بود: {type(exc).__name__}: {exc}")
    ...
    def init_db(self):
        ...
        try:
            ...
            default_settings = {
                ...
                "comment": "off", "text": "first !", "typing": "off",
                ...
                "online_status": "off", "typing_action": "off", "typing_duration": "5",
                "auto_forward": "off", "save_timed_photos": "off",
            ...
            self.apply_one_time_feature_defaults()
...
✅ **اکانت با موفقیت غیرفعال شد**
    ...
    async def register_settings_handlers(self):
        ...
        @self.client.on(events.NewMessage(pattern=r'^(?:\.)?(?:online|typing|secretary|autoreply|autoforward|timename|timebio|save|schedule) (?:on|off|روشن|خاموش)$'))
        ...
        @self.client.on(events.NewMessage(outgoing=True))
        async def persian_settings_text_handler(event):
            """Map the documented Persian text commands to existing settings only."""
            try:
                if not self.is_owner_outgoing_event(event):
                    return
                text = (event.raw_text or "").strip().replace("ي", "ی").replace("ك", "ک")
                low = text.lower()
                settings = self.get_data()

                def state_word(value):
                    return "on" if value in {"روشن", "on"} else "off"

                simple = {
                    "حالت آنلاین": "online_status",
                    "حالت تایپینگ": "typing_action",
                    "منشی عمومی": "secretary",
                    "منشی": "offline_reply_enabled",
                    "ذخیره عکس زمان‌دار": "save_timed_photos",
                    "ساعت اسم": "timename",
                    "ساعت بیو": "timebio",
                    "ارسال خودکار": "scheduled_message_enabled",
                }
                for root, key in simple.items():
                    if low.startswith(root + " "):
                        value = low[len(root):].strip()
                        if value in {"روشن", "خاموش", "on", "off"}:
                            settings[key] = state_word(value)
                            ...
                            if key == "online_status":
                                ...
                                await self.force_time_update()
                            await event.reply(f"✅ {root} {'فعال' if settings[key] == 'on' else 'غیرفعال'} شد.")
                            # پیام دستور عمداً حذف نمی‌شود.
                            return

                m = re.fullmatch(r"تنظیم زمان تایپینگ\s+(\d+)", text)
                if m:
                    settings["typing_duration"] = m.group(1)
                    self.put_data(settings)
                    await event.reply(f"✅ مدت زمان تایپینگ روی {m.group(1)} ثانیه تنظیم شد.")
                    return

                m = re.fullmatch(r"تنظیم فونت ساعت اسم\s+(\d+)", text)
                if m and 1 <= int(m.group(1)) <= 10:
                    settings["timename_font"] = m.group(1); self.put_data(settings)
                    await event.reply("✅ فونت ساعت اسم تنظیم شد."); return

                m = re.fullmatch(r"تنظیم فونت ساعت بیو\s+(\d+)", text)
                if m and 1 <= int(m.group(1)) <= 10:
                    settings["timebio_font"] = m.group(1); self.put_data(settings)
                    await event.reply("✅ فونت ساعت بیو تنظیم شد."); return

                m = re.fullmatch(r"تنظیم منشی عمومی\s+([\s\S]+)", text)
                if m:
                    settings["secretary_fallback_text"] = m.group(1).strip()[:3500]
                    self.put_data(settings)
                    await event.reply("✅ متن منشی عمومی ذخیره شد."); return

                m = re.fullmatch(r"تنظیم متن منشی\s+([\s\S]+)", text)
                if m:
                    settings["offline_reply_text"] = m.group(1).strip()[:3500]
                    self.put_data(settings)
                    await event.reply("✅ متن منشی آفلاین ذخیره شد."); return

                m = re.fullmatch(r"تنظیم متن امضا\s+([\s\S]+)", text)
                if m:
                    settings["outgoing_signature_text"] = m.group(1).strip()[:300]
                    self.put_data(settings)
                    await event.reply("✅ متن امضا ذخیره شد."); return

            except Exception as exc:
                print(f"خطا در دستور تنظیمات فارسی برای {self.phone}: {exc}")
