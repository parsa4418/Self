    ...
    async def register_handlers(self):
        ...
        async def handle_admin_commands(event):
            try:
                ...
                command_text = message_text[1:].strip() if message_text.startswith('.') else message_text
            ...
            try:
                ...
                for key, handler in handlers.items():
                    ...
                    if key_normalized.startswith("."):
                        key_normalized = key_normalized[1:].strip()
                    if command_text == key_normalized:
                        await handler(event)
                        return

                if command_text.startswith('info') or command_text.startswith('اطلاعات'):
                    await self.info_handler(event)
                    return

                # دستورهای اختصاصی قابلیت‌ها را به FeatureEngine بسپار.
                if self.feature_engine is not None:
                    handled = await self.feature_engine.handle_command(event)
                    if handled:
                        return
