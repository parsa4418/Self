        ...
        try:
            ...
            try:
                ...
                if me:
                    self.owner_id = me.id
                    self.connection_retries = 0
                    # Authentication + a live Telegram connection are enough
                    # for the manager process to consider this selfbot alive.
                    # The remaining startup work contains optional network
                    # operations (channel joins, startup messages, feature
                    # loading) and must not block activation long enough for
                    # main_bot.py to terminate a healthy child process.
                    write_runtime_status(
                        self.status_file,
                        "ready",
                        "Session معتبر است و Client احراز هویت شده است؛ ادامه راه‌اندازی در حال انجام است.",
                    )
